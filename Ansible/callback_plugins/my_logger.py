...
class CallbackModule(CallbackBase):
    ...
    def _escape(value):
        """
        Экранирует спецсимволы для значений лейблов Prometheus.
        В значениях лейблов нужно экранировать: \ " и перевод строки.
        """
        return (str(value)
                .replace('\\', '\\\\')
                .replace('"', '\\"')
    ...
    def _write_atomic(filepath, content):
        ...
        ЗАЧЕМ ЭТО НУЖНО:
        Если мы просто откроем файл и начнём писать,
        Node Exporter может прочитать его В СЕРЕДИНЕ записи
        (когда файл ещё наполовину пустой) и получить
        битые данные → он проигнорирует весь файл.
        
        Атомарная запись:
        1. Пишем во ВРЕМЕННЫЙ файл в той же директории.
        2. Когда запись завершена, атомарно ПЕРЕИМЕНОВЫВАЕМ
           временный файл в целевой (os.rename на Linux — атомарная операция).
        3. Node Exporter либо видит старый полный файл,
           либо мгновенно видит новый полный файл.
           Промежуточного "наполовину записанного" состояния не существует.
        """
        dir_name = os.path.dirname(filepath)
        try:
            # mkstemp создаёт временный файл и возвращает (fd, path)
            ...
            try:
                ...
                # Если что-то пошло не так — убираем за собой временный файл
                os.unlink(tmp_path)
                raise
        except Exception as e:
            # Не используем self._display здесь, потому что метод статический.
            # Ошибка всплывёт в логах Ansible.
